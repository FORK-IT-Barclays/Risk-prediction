...
from realtime_risk_engine.src.config import META_ZONE_GRID as ZONE_GRID, META_TIER_MAPPING
...
for r in results:
    ...
    print(f"    Intervention Tier: {traj['tier']}")
...
print(f"{'=' * 70}")
print(f"  {'Customer':<12} {'Name':<35} {'Tier':<12} {'Zone':<22}")
print(f"  {'-'*12} {'-'*35} {'-'*12} {'-'*22}")
...
for r in results:
    ...
    print(f"  {cust['id']:<12} {name:<35} {traj['tier']:<12} {traj['zone']:<22}")
...
zone_order = [
    "SOLID_STABILITY", "LINEAR_RECOVERY", "TOTAL_REHAB",
    "LINEAR_WORSENING", "RECOVERY_STALL", "LATE_STABILITY",
    "REACHING_PEAK", "EARLY_VOLATILITY", "EXPONENTIAL_CRASH",
]

for zone in zone_order:
    count = zone_counts.get(zone, 0)
    bar = "X" * (count * 5) if count > 0 else ""
    # Map tier for urgency display
    tier = META_TIER_MAPPING.get(zone, "TIER_3_GREEN")
    urgency = "HIGH" if "RED" in tier else ("MEDIUM" if "YELLOW" in tier else "LOW")
...
# Categorize
critical = [r for r in results if r["trajectory"]["tier"] == "TIER_1_RED"]
watch = [r for r in results if r["trajectory"]["tier"] == "TIER_2_YELLOW"]
safe = [r for r in results if r["trajectory"]["tier"] == "TIER_3_GREEN"]
