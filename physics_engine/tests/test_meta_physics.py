...
assert_test("DTT = Stable / Improving label", result["days_to_default"] == "Stable / Improving",
            f"got {result['days_to_default']}")
assert_test("zone contains RECOVERING", "RECOVERING" in result["zone"],
            f"got {result['zone']}")
...
assert_test("trend = STABLE", result["trend"] == "STABLE",
            f"got {result['trend']}")
assert_test("zone = SAFE_STABLE", result["zone"] == "SAFE_STABLE",
            f"got {result['zone']}")
...
assert_test("single: zone defaults to SAFE_STABLE", result_single["zone"] == "SAFE_STABLE",
...
assert_test("DTT = 0 (already critical)", result["days_to_default"] == 0,
            f"got {result['days_to_default']}")
assert_test("zone contains CRITICAL", "CRITICAL" in result["zone"],
            f"got {result['zone']}")
...
test_cases = [
    (0.20, -0.01, "SAFE_RECOVERING"),
    (0.20, 0.000, "SAFE_STABLE"),
    (0.20, 0.01,  "SAFE_WORSENING"),
    (0.55, -0.01, "WATCH_RECOVERING"),
    (0.55, 0.000, "WATCH_STABLE"),
    (0.55, 0.01,  "WATCH_WORSENING"),
    (0.80, -0.01, "CRITICAL_RECOVERING"),
    (0.80, 0.000, "CRITICAL_STABLE"),
    (0.80, 0.01,  "CRITICAL_SPIRAL"),
]
...
required_keys = [
    ...
    "days_to_default", "current_risk", "zone", "trajectory_window", "analysed_at"
]
...
assert_test("burst: current_risk is correct", result["current_risk"] == 0.44,
            f"got {result['current_risk']}")
assert_test("burst: zone is assigned based on level", result["zone"] == "SAFE_STABLE",
            f"got {result['zone']}")
